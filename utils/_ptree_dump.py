...
def generate_directory_tree(path, filename='_project-tree.txt', exclude=None):
    ...
    for root, dirs, files in os.walk(path, topdown=True):
        # Exclude directories that are in the exclude list or start with '_' or '.'
        ...
        subindent = '│   ' * (level + 1)
        for f in files:
            # Exclude files that start with '_' or '.'
            if f.startswith(('_', '.')):
                continue
...
if __name__ == "__main__":
    ...
    generate_directory_tree(
        project_root,
        filename='_ptree_dump.txt',
        exclude=['__pycache__', '.git', 'build', 'venv', '_static', 'scratch']
    )
