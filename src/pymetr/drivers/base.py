...
from .connections import PyVisaConnection
from .connections import RawSocketConnection
...
class Property(ABC):
    ...
    def __init__(self, cmd_str: str, doc_str: str = "", access: str = "read-write", join_char: str = " "):
        ...
        # Add signal for property changes
        self.value_changed = Signal(object)  # Value that changed
        self.path_changed = Signal(str, object)  # (property_path, new_value)
    ...
    def __set__(self, instance, value):
        ...
        # Emit change signals
        try:
            # Get the property path by inspecting the instance
            path = self._get_property_path(instance)
            self.value_changed.emit(value)
            self.path_changed.emit(path, value)
        except Exception as e:
            logger.error(f"Error emitting property change: {e}")

    def _get_property_path(self, instance):
        """Get the full property path for this property on the instance"""
        # This could be implemented by walking up the instance hierarchy
        # and building the path (e.g. "channel[1].frequency")
        # For now a simple implementation:
        for name, value in type(instance).__dict__.items():
            if value is self:
                return f"{instance.__class__.__name__.lower()}.{name}"
        return None
